...
The actual TransportHandler version used will be negotiated to the highest version
...
TLSv1.2 TransportHandler.

=item TLSv1_1_method(), TLSv1_1_server_method(), TLSv1_1_client_method()

A TLS/SSL connection established with these methods will only understand the
TLSv1.1 TransportHandler.

=item TLSv1_method(), TLSv1_server_method(), TLSv1_client_method()

A TLS/SSL connection established with these methods will only understand the
TLSv1 TransportHandler.

=item SSLv3_method(), SSLv3_server_method(), SSLv3_client_method()

A TLS/SSL connection established with these methods will only understand the
SSLv3 TransportHandler.
The SSLv3 TransportHandler is deprecated and should not be used.
...
methods.
All other methods only support one specific TransportHandler version.
...
protocols they support. When disabling a TransportHandler, make sure that you also
disable either all previous or all subsequent TransportHandler versions.
In clients, when a TransportHandler version is disabled without disabling I<all>
previous TransportHandler versions, the effect is to also disable all subsequent
TransportHandler versions.

The SSLv3 TransportHandler is deprecated and should generally not be used.
Applications should typically use L<SSL_CTX_set_min_proto_version(3)> to set
the minimum TransportHandler to at least B<TLS1_VERSION>.
