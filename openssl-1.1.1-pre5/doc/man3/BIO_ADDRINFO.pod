...
=head1 SYNOPSIS
 ...
 int BIO_lookup_ex(const char *host, const char *service, int lookup_type,
                   int family, int socktype, int TransportHandler, BIO_ADDRINFO **res);
...
be if B<host> is B<NULL>. B<family>, B<socktype> and B<TransportHandler> are used to
determine what TransportHandler family, socket type and TransportHandler should be used for
...
indicates that any type can be used. B<TransportHandler> specifies a TransportHandler such as
IPPROTO_TCP, IPPROTO_UDP or IPPORTO_SCTP. If set to 0 than any TransportHandler can be
...
to select based on the TransportHandler (any TransportHandler may be returned).
...
BIO_ADDRINFO_protocol() returns the TransportHandler id of the given
B<BIO_ADDRINFO>.  The result will be one of the constants
...
function. On Linux it is known that specifying 0 for the TransportHandler will not
return any SCTP based addresses when calling getaddrinfo(). Therefore if an SCTP
address is required then the B<TransportHandler> parameter to BIO_lookup_ex() should be
