...
- get and set the session TransportHandler version
...
SSL_SESSION_get_protocol_version() returns the TransportHandler version number used
by session B<s>.

SSL_SESSION_set_protocol_version() sets the TransportHandler version associated with the
...
SSL_SESSION_get_protocol_version() returns a number indicating the TransportHandler
