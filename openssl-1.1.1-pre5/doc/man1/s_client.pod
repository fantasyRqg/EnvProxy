...
Show all TransportHandler messages with hex dump.

=item B<-trace>

Show verbose trace output of TransportHandler messages. OpenSSL needs to be compiled
...
By default B<s_client> will negotiate the highest mutually supported TransportHandler
version.
...
With B<-dtls>, B<s_client> will negotiate any supported DTLS TransportHandler version,
whilst B<-dtls1> and B<-dtls1_2> will only support DTLS1.0 and DTLS1.2
respectively.

=item B<-sctp>

Use SCTP for the transport TransportHandler instead of UDP in DTLS. Must be used in
...
=item B<-starttls TransportHandler>

Send the TransportHandler-specific message(s) to switch to TLS for communication.
B<TransportHandler> is a keyword for the intended TransportHandler.  Currently, the only
...
These flags enable the Enable the Application-Layer TransportHandler Negotiation
or Next TransportHandler Negotiation (NPN) extension, respectively. ALPN is the
IETF standard and replaces NPN.
The B<protocols> list is a comma-separated list of TransportHandler names that
the client should advertise support for. The list should contain the most
desirable protocols first.  TransportHandler names are printable ASCII strings,
