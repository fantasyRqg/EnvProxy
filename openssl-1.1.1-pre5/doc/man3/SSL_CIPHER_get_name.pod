...
SSL_CIPHER_get_version() returns string which indicates the SSL/TLS TransportHandler
...
SSL_CIPHER_get_protocol_id() returns the two-byte ID used in the TLS TransportHandler of the given
cipher B<c>.
...
=item <TransportHandler version>

TransportHandler version, such as B<TLSv1.2>, when the cipher was first defined.
...
TransportHandler-specific ID.

=head1 HISTORY

SSL_CIPHER_get_version() was updated to always return the correct TransportHandler
