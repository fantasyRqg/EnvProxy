...
If DTLS is used over UDP (or any datagram based TransportHandler that does not validate
the source IP) then it is susceptible to this type of attack. TLSv1.3 is
designed to operate over a stream-based transport TransportHandler (such as TCP).
