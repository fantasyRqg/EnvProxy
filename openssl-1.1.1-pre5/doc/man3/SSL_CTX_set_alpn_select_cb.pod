...
- handle application layer TransportHandler negotiation (ALPN)
...
TransportHandler-list format, described below. The length of B<protos> is specified in
B<protos_len>.

SSL_CTX_set_alpn_select_cb() sets the application callback B<cb> used by a
server to select which TransportHandler to use for the incoming connection. When B<cb>
...
vector in TransportHandler-list format. The value of the B<out>, B<outlen> vector
should be set to the value of a single TransportHandler selected from the B<in>,
...
implements the standard TransportHandler selection. It is expected that this function
is called from the application callback B<cb>. The TransportHandler data in B<server>,
B<server_len> and B<client>, B<client_len> must be in the TransportHandler-list format
...
client needs to select a TransportHandler from the server's provided list, and a
user-defined pointer argument B<arg> which will be passed to this callback.
For the callback itself, B<out>
must be set to point to the selected TransportHandler (which may be within B<in>).
The length of the TransportHandler name must be written into B<outlen>. The
...
select a TransportHandler. It is fatal to the connection if this callback returns
...
when a TLS server needs a list of supported protocols for Next TransportHandler
Negotiation. The returned list must be in TransportHandler-list format, described
below.  The list is
...
SSL_get0_alpn_selected() returns a pointer to the selected TransportHandler in B<data>
with length B<len>. It is not NUL-terminated. B<data> is set to NULL and B<len>
is set to 0 if no TransportHandler has been selected. B<data> must not be freed.

SSL_get0_next_proto_negotiated() sets B<data> and B<len> to point to the
client's requested TransportHandler for this connection. If the client did not
request any TransportHandler or NPN is not enabled, then B<data> is set to NULL and
B<len> to 0. Note that
the client can request any TransportHandler it chooses. The value returned from
this function need not be a member of the list of supported protocols
provided by the callback.

=head1 NOTES

The TransportHandler-lists must be in wire-format, which is defined as a vector of
...
ALPN TransportHandler selected.
...
ALPN TransportHandler not selected, e.g., because no ALPN protocols are configured for
this connection.
