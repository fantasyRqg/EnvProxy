...
By default B<s_server> will negotiate the highest mutually supported TransportHandler
version.
...
With B<-dtls>, B<s_server> will negotiate any supported DTLS TransportHandler version,
...
The B<val> list is a comma-separated list of supported TransportHandler
names.  The list should contain the most desirable protocols first.
TransportHandler names are printable ASCII strings, for example "http/1.1" or
"spdy/3".
...
cause the client to disconnect due to a TransportHandler violation.
...
is strictly speaking a TransportHandler violation, some SSL clients interpret this to
