...
int DnsSession::onTunDown(SessionInfo *sessionInfo, DataBuffer *downData) {
    char qname[DNS_QNAME_MAX + 1];
    uint16_t qtype;
    uint16_t qclass;
    if (getDnsQuery(sessionInfo, downData->data, downData->size, &qtype, &qclass, qname) >= 0) {
        ALOGD("DNS query qtype %d qclass %d name %s", qtype, qclass, qname);
        return onTunUp(sessionInfo, downData);
    } else {
        // drop dns nonstandard pkt
        ALOGW("drop nonstandard dns pkt");
        return 0;
    }
...
int DnsSession::onTunUp(SessionInfo *sessionInfo, DataBuffer *upData) {
    return prev->onTunUp(sessionInfo, upData);
}

int DnsSession::onSocketDown(SessionInfo *sessionInfo, DataBuffer *downData) {
    return onSocketUp(sessionInfo, downData);
}

int DnsSession::onSocketUp(SessionInfo *sessionInfo, DataBuffer *upData) {
    parseDnsResponse(sessionInfo, upData->data, (upData->size));
    return prev->onSocketUp(sessionInfo, upData);
}

int32_t getQname(const uint8_t *data, const size_t datalen, uint16_t off, char *qname) {
    *qname = 0;
    ...
    while (len) {
        if (len & 0xC0) {
            ...
            len = *(data + ptr);
            ALOGD("DNS qname compression ptr %d len %d", ptr, len);
            if (!c) {
                c = 1;
                off += 2;
            }
    ...
    if (len > 0 || noff == 0) {
        ALOGE("DNS qname invalid len %d noff %d", len, noff);
        return -1;
    }
...
void DnsSession::parseDnsResponse(const SessionInfo *sessionInfo,
                                  const uint8_t *data, size_t datalen) {

    if (datalen < sizeof(struct dns_header) + 1) {
        ALOGW("DNS response length %zu", datalen);
        return;
    }
    ...
    if (dns->qr == 1 && dns->opcode == 0 && qcount > 0 && acount > 0) {
        ALOGD("DNS response qcount %d acount %d", qcount, acount);
        if (qcount > 1)
            ALOGW("DNS response qcount %d acount %d", qcount, acount);
        ...
        for (int q = 0; q < 1; q++) {
            off = getQname(data, datalen, (uint16_t) off, name);
            if (off > 0 && off + 4 <= datalen) {
                ...
                if (q == 0) {
                    ...
                    ALOGD("DNS question %d qtype %d qclass %d qname %s",
                          q, qtype, qclass, qname);
                }
                off += 4;
            } else {
                ALOGW("DNS response Q invalid off %d datalen %zu", off, datalen);
                return;
            }
        }
...
//                        dns_resolved(args, qname, name, rd, ttl);
                        ALOGD("DNS answer %d qname %s qtype %d ttl %d data %s",
                              a, name, qtype, ttl, rd);

                    } else
                        ALOGD("DNS answer %d qname %s qclass %d qtype %d ttl %d length %d",
                              a, name, qclass, qtype, ttl, rdlength);

                    off += rdlength;
                } else {
                    ALOGW("DNS response A invalid off %d rdlength %u datalen %zu",
                          off, rdlength, datalen);
                    return;
                }
            } else {
                ALOGW("DNS response A invalid off %d datalen %zu", off, datalen);
                return;
            }
        }
...
//        }
        ...
        ALOGW("DNS response qr %d opcode %d qcount %d acount %d",
              dns->qr, dns->opcode, qcount, acount);
}


int DnsSession::getDnsQuery(const SessionInfo *sessionInfo,
                            const uint8_t *data, const size_t datalen,
                            uint16_t *qtype, uint16_t *qclass, char *qname) {
    if (datalen < sizeof(struct dns_header) + 1) {
        ALOGW("DNS query length %zu", datalen);
        return -1;
    }
    ...
    if (dns->qr == 0 && dns->opcode == 0 && qcount > 0) {
        if (qcount > 1)
            ALOGW("DNS query qcount %d", qcount);

        // http://tools.ietf.org/html/rfc1035
        int off = getQname(data, datalen, sizeof(struct dns_header), qname);
        if (off > 0 && off + 4 == datalen) {
            *qtype = ntohs(*((uint16_t *) (data + off)));
            *qclass = ntohs(*((uint16_t *) (data + off + 2)));
            return 0;
        } else
            ALOGW("DNS query invalid off %d datalen %zu", off, datalen);
    }

    return -1;
}

//int check_domain(const SessionInfo *sessionInfo,
//                 const uint8_t *data, const size_t datalen,
...
//            ) {
//
//        ALOGI("DNS query type %d name %s blocked", qtype, name);
//
//        // Build response
//        size_t rlen = datalen + sizeof(struct dns_rr) + (qtype == DNS_QTYPE_A ? 4 : 16);
//        uint8_t *response = sessionInfo->balloc(rlen);
//
//        // Copy header & query
...
//            ALOGW("UDP DNS write error %d: %s", errno, strerror(errno));
//
//        free(response);
//
//        return 1;
