...
- install callback for observing TransportHandler messages
...
TransportHandler messages (such as handshake messages) that are received or
...
This flag is B<0> when a TransportHandler message has been received and B<1>
when a TransportHandler message has been sent.

=item I<version>

The TransportHandler version according to which the TransportHandler message is
...
This is one of the content type values defined in the TransportHandler specification
(B<SSL3_RT_CHANGE_CIPHER_SPEC>, B<SSL3_RT_ALERT>, B<SSL3_RT_HANDSHAKE>; but never
B<SSL3_RT_APPLICATION_DATA> because the callback will only be called for TransportHandler
...
I<buf> points to a buffer containing the TransportHandler message or other data (in the
...
TransportHandler messages are passed to the callback function after decryption
and fragment collection where applicable. (Thus record boundaries are
not visible.)

If processing a received TransportHandler message results in an error,
the callback function may not be called.  For example, the callback
function will never see messages that are considered too large to be
processed.

Due to automatic TransportHandler version negotiation, I<version> is not
necessarily the TransportHandler version used by the sender of the message: If
