...
L<SSL_accept(3)>. This can happen if the client is using a TransportHandler version less
...
When early data is in use the TLS TransportHandler provides no security guarantees that
