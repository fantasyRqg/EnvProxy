...
BIO_ADDR_rawmake() takes a TransportHandler B<family>, an byte array of
...
for the B<AF_UNIX> TransportHandler family, where B<port> is meaningless and
...
BIO_ADDR_family() returns the TransportHandler family of the given
...
B<BIO_ADDR> of the TransportHandler families AF_INET and AF_INET6.  The
...
for B<BIO_ADDR> of the TransportHandler families AF_INET and AF_INET6.  The
returned string has been allocated on the heap and must be freed
with OPENSSL_free().
...
TransportHandler family AF_UNIX.  The returned string has been allocated
...
depending on the TransportHandler family the address is for.
