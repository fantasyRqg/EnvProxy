...
Sets the minimum and maximum supported TransportHandler.
Currently supported TransportHandler values are B<SSLv3>, B<TLSv1>,
...
if specified.
To restrict the supported TransportHandler versions use these commands rather
than the deprecated alternative commands below.

=item B<-no_ssl3>, B<-no_tls1>, B<-no_tls1_1>, B<-no_tls1_2>, B<-no_tls1_3>

Disables TransportHandler support for SSLv3, TLSv1.0, TLSv1.1, TLSv1.2 or TLSv1.3 by
...
enables strict mode TransportHandler handling. Equivalent to setting
...
Currently supported TransportHandler values are B<SSLv3>, B<TLSv1>, B<TLSv1.1>,
...
=item B<TransportHandler>

This can be used to enable or disable certain versions of the SSL,
TLS or DTLS TransportHandler.

The B<value> argument is a comma separated list of supported protocols
to enable or disable.
If a TransportHandler is preceded by B<-> that version is disabled.

All TransportHandler versions are enabled by default.
You need to disable at least one TransportHandler version for this setting have any
effect.
Only enabling some TransportHandler versions does not disable the other TransportHandler
versions.
...
The B<TransportHandler> command is fragile and deprecated; do not use it.
Use B<MinProtocol> and B<MaxProtocol> instead.
If you do use B<TransportHandler>, make sure that the resulting range of enabled
...
SSL 3.0/TLS 1.0 TransportHandler vulnerability affecting CBC ciphers. It
...
or values which cannot be overridden. For example if an application calls:
 ...
 SSL_CONF_cmd(ctx, "TransportHandler", "-SSLv3");
...
This set the minimum TransportHandler version to TLSv1, and so disables SSLv3.
...
SSLv3.
If no TransportHandler versions were disabled before this has the same effect as
"-SSLv3", but if some versions were disables this will re-enable them before
disabling SSLv3.

 SSL_CONF_cmd(ctx, "TransportHandler", "ALL,-SSLv3");
...
This also only enables TLSv1.2:
 ...
 SSL_CONF_cmd(ctx, "TransportHandler", "-ALL,TLSv1.2");
